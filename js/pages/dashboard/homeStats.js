...
function loadCaseDepartments() {
    ...
    $.when(statsCases(parameters, conditions, order, first, last, select)).done(function(stats) {
        //console.log(graph);
        if(stats.count == 0) {
            graph.push(['', 0, colors[colorCount]]);
        }
        $.each(stats.results, function(i, result) {
...
function loadCaseStats() {
    ...
    $.each(results, function(item, contents) {
        ...
        if(item=='Open Cases') {
            conditions='t.assigned_to = :assigned_to AND date_opened <= :start_time AND (date_closed = "" OR date_closed IS NULL OR date_closed > :end_time) AND :title=:title';
        }
        if(item=='New Cases') {
            conditions='t.assigned_to = :assigned_to AND date_opened >= :start_time AND date_opened <=:end_time AND :title=:title';
        }
        if(item=='Closed Cases') {
            conditions='t.assigned_to = :assigned_to AND date_closed >= :start_time AND date_closed <= :end_time AND :title=:title';
        }

        var iterations=parseInt(((startTimeStamp-earlydate)/(86400*7))+1);
            ...
            (function(tindex) {
                ...
                $.when(statsCases(parameters, conditions, order, first, last, null)).done(function(stats) {
                    ...
                    if(stats.parameters[":title"]=="Open Cases") {
                        ...
                        graph[tindex][2]=parseInt(stats.results[0]['total']);
                    } else if(stats.parameters[":title"]=="New Cases") {
                        graph[tindex][3]=parseInt(stats.results[0]['total']);
                    }
...
function loadAllCaseDates() {
    $('#dashboardStatistics').html('<img src="images/logo_flip.gif" />');
    
    var parameters={};
    //parameters[':assigned_to']=globals.user_id;
    var conditions='is_closed != 1';
...
function loadAllCaseDepartments() {
    $('#dashboardStatistics').html('<img src="images/logo_flip.gif" />');
...
function loadAllCaseStats() {
    ...
    $.each(results, function(item, contents) {
        ...
        //Note the ":title = :title" is a hack/trick designed to return the item being run in the parameter section, to avoid confusion in compilation of results
        if(item=='Open Cases') {
            conditions='date_opened <= :start_time AND (date_closed = "" OR date_closed IS NULL OR date_closed > :end_time) AND :title=:title';
        }
        if(item=='New Cases') {
            conditions='date_opened >= :start_time AND date_opened <=:end_time AND :title=:title';
        }
        if(item=='Closed Cases') {
            conditions='date_closed >= :start_time AND date_closed <= :end_time AND :title=:title';
        }
